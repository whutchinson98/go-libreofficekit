...
static char* lok_dlerror(void) {
  ...
  FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                 ...
                 NULL, GetLastError(), 0, reinterpret_cast<LPSTR>(&buf), 0,
                 NULL);
  return buf;
}
...
static void* lok_dlopen(const char* install_path, char** _imp_lib) {
  ...
  *_imp_lib = NULL;
  ...
  imp_lib_size =
      partial_length + sizeof(TARGET_LIB) + sizeof(TARGET_MERGED_LIB) + 2;
  ...
  if (!dlhandle) {
    ...
    strncpy(imp_lib + partial_length, TARGET_MERGED_LIB,
            imp_lib_size - partial_length);
...
typedef LibreOfficeKit*(LokHookFunction2)(const char* install_path,
                                          const char* user_profile_url);

typedef int(LokHookPreInit)(const char* install_path,
                            const char* user_profile_url);

typedef int(LokHookPreInit2)(const char* install_path,
                             const char* user_profile_url,
                             LibreOfficeKit** kit);
...
static LibreOfficeKit* lok_init_2(const char* install_path,
                                  const char* user_profile_url) {
#if !defined(IOS) && !defined(ANDROID)
  void* dlhandle;
  char* imp_lib;
  ...
  if (!pSym2) {
    if (user_profile_url != NULL) {
      fprintf(stderr,
              "the LibreOffice version in '%s' does not support passing a user "
              "profile to the hook function\n",
              imp_lib);
...
static LOK_TOLERATE_UNUSED LibreOfficeKit* lok_init(const char* install_path) {
  return lok_init_2(install_path, NULL);
}

#if !defined(IOS)
static LOK_TOLERATE_UNUSED int lok_preinit(const char* install_path,
                                           const char* user_profile_url) {
  void* dlhandle;
  char* imp_lib;
  LokHookPreInit* pSym;
